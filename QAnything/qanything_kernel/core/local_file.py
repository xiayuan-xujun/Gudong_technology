from qanything_kernel.core.split_laws import split_doc
...
class LocalFile:
    ...
    def split_file_to_docs(self, ocr_engine: Callable, sentence_size=SENTENCE_SIZE,
        ...
        if self.url:
            ...
            # texts_splitter = ChineseTextSplitter(pdf=False, sentence_size=sentence_size)
            # docs = loader.load_and_split(texts_splitter)
            debug_logger.info(f'处理文件:.docx，地址：{self.file_path}')
            docs = split_doc(self.file_path)
        ...
        # 不是csv，xlsx和FAQ的文件，需要再次分割， docx文件额外处理。
        if not self.file_path.lower().endswith(".csv") and not self.file_path.lower().endswith(".xlsx") and not self.file_path == 'FAQ'and not self.file_path.lower().endswith(".docx"):
            ...
            if self.file_path.lower().endswith(".pdf"):
                ...
                docs = text_splitter.split_documents(new_docs)      # 切割文本，可以重叠的切割。
            ...
            for doc in docs:
                if not new_docs:
                    ...
                    last_doc = new_docs[-1]
                    if len(last_doc) + len(doc) < min_length:
                        last_doc += '\n' + doc
                    else:
                        new_docs.append(doc)
            docs = new_docs
