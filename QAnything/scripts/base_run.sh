...
openai_api_context_length="2048"
...
echo "CUDA_VISIBLE_DEVICES=2 python3 -m qanything_kernel.qanything_server.sanic_api --host 0.0.0.0 --port $qanything_port --model_size $model_size $use_cpu_option $use_openai_api_option ${openai_api_base:+--openai_api_base "$openai_api_base"} ${openai_api_key:+--openai_api_key "$openai_api_key"} ${openai_api_model_name:+--openai_api_model_name "$openai_api_model_name"} ${openai_api_context_length:+--openai_api_context_length "$openai_api_context_length"} ${workers:+--workers "$workers"}"

sleep 5
# 启动qanything-server服务
CUDA_VISIBLE_DEVICES=2 python3 -m qanything_kernel.qanything_server.sanic_api --host 0.0.0.0 --port $qanything_port --model_size $model_size \
